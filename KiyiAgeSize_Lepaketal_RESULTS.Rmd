...
The mean lengths-at-ages 5, 6, and 11 differed signficantly among regions (Figure 7).  Fish from East Michigan were significantly longer (p < `r kPvalue(grow5_max.sigp,digits=3,include.p=FALSE)`) at age-5 than age-5 fish from other regions which did not differ significantly (p > `r kPvalue(grow5_min.nsigp,digits=3,include.p=FALSE)`).  Fish from North Ontario were significantly shorter than fish from South Ontario and East Michigan at age 6 (p < `r kPvalue(grow6_max.sigp,digits=3,include.p=FALSE)`) and 11 (p < `r kPvalue(grow11_max.sigp,digits=3,include.p=FALSE)`) with no other significant differences at these ages (p > `r kPvalue(min(c(grow6_min.nsigp,grow11_min.nsigp)),digits=3,include.p=FALSE)`).  Kiyi from the 2003 and 2009 year-classes exhbited similar growth trajectories during at least the first five years of life (Figure 7).
